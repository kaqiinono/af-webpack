...
const pageVersion = process.env.VERSION || 'v1';

function getConfig(opts = {}) {
  if (process.env.PAGE) {
    const commonsName = `${process.env.PAGE}/${pageVersion}/common`;
    opts.commons[0].name = commonsName;
    opts.entry = {
      [commonsName]: opts.entry.common,
      [`${process.env.PAGE}/${pageVersion}/index`]: `./src/entry/${process.env.PAGE}.js`
    };
  }
  ...
  const outputPath = opts.outputPath ? (0, _path.resolve)(opts.cwd, opts.outputPath) : (0, _path.resolve)(opts.cwd, 'dist' + `${process.env.PAGE ? '_' + process.env.PAGE : ''}`); // Copy files in public to outputPath
  ...
  let asyncFile = `[name]${jsHash}.async.js`
  if (process.env.PAGE) {
    asyncFile = `${process.env.PAGE}/${process.env.VERSION || 'v1'}/[name]${jsHash}.async.js`;
  }
  const config = {
    bail: !isDev,
